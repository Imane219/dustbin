from __future__ import print_function #use the print function in 3.x
...
def build_cfg_and_analyze():
    ...
    with open(g_disasm_file, 'r') as disasm_file:
        ...
        tokens = tokenize.generate_tokens(disasm_file.readline) ##https://blog.csdn.net/lnotime/article/details/104906662
...
def get_init_global_state(path_conditions_and_vars):
    ...
    init_ia = deposited_value = sender_address = receiver_address = gas_price = origin = currentCoinbase = currentNumber = currentDifficulty = currentGasLimit = callData = None
    global init_is
    init_is=None
...
def sym_exec_block(params, block, pre_block, depth, func_call, current_func_name):
    ...
    if analysis["money_flow"] not in money_flow_all_paths:
        # print(path_conditions_and_vars["path_condition"])
...
def sym_exec_ins(params, block, instr, func_call, current_func_name):
    ...
    #dic={}
    dic=update_analysis(analysis, opcode, stack, mem, global_state, path_conditions_and_vars, solver,g_src_map)
    ...
    #
        ...
        if len(stack) > 0:
            ...
            if isAllReal(address, current_miu_i) and address in mem:
                ...
                print(address,current_miu_i,new_var)
        ...
        if len(stack) > 1:
            ...
            if isAllReal(stored_address, current_miu_i):
                ...
                mem[str(stored_address)] = stored_value
            global_state["miu_i"] = current_miu_i
            #print(stored_value,temp_value,stored_value)
        else:
            raise ValueError('STACK underflow')
    elif opcode == "SLOAD":
        if len(stack) > 0:
...
def detect_time_dependency():
    ...
    global timestamp
    global blocknumber
    global banlance_timestamp
    timestamp=None
    blocknumber=None
    banlance_timestamp=None
    TIMESTAMP_VAR = "IH_s"
    is_dependant = False
    pcs = []
    dict_timestamp={}
    ...
    for i, cond in enumerate(path_conditions):
        ...
        if(is_dependant):
            ...
            solver.add(cond)
            if solver.check()==sat:
                # if(solver.model()['init_Is']!=0):
                #     print solver.model()['init_Is']
                a=solver.model()
                print (a)
            s=str(a)
            s=s[1:]
            ...
            for i in range(len(list)/3):
                dict_timestamp[list[3*i]]=list[3*i+2][:-1]
            solver.pop()
            #print (dict_timestamp)

    for k,v in dict_timestamp.items():
        if(k=='IH_s'):
            timestamp=v
        elif(k=='IH_i'):
            blocknumber=v
        elif(k=='init_Is'):
            banlance_timestamp=v


    func_params_value_timestamp={}
    if(dict_timestamp):
        for func in g_abi:
            inputs=func['inputs']
            name=func['name']
            d1={}
            for dic in inputs:
                d1[str(dic['name'])]=None
                for k,v in dict_timestamp.items():
                    if(str(dic['name'])==k):
                        d1[k]=v
            func_params_value_timestamp[str(name)]=d1
    print("2")
    print(func_params_value_timestamp)
    ...
    if global_params.REPORT_MODE:
        ...
        with open(report_file, 'w') as rfile:
            if is_dependant:
                ...
                rfile.write("no\n")
    return func_params_value_timestamp
...
def detect_money_concurrency():
    ...
    concurrency_paths = []
    flows = []
    for flow in money_flow_all_paths:#find one
        i += 1
        if len(flow) == 1:
        ...
        for j in range(i, n):
            ...
            if len(jflow) == 1:
                continue
            if is_diff(flow, jflow):#path[i-1] & path[> i-1]
...
def detect_reentrancy():
    ...
    global results
    global reentrancy
    global function_params_vaule
    global dict_reentrancy
    #print(dict_reentrancy)
    global g_abi
    global banlance_reentrancy
    banlance_reentrancy=None
    if(dict_reentrancy):
        for k,v in dict_reentrancy.items():
            if(k=='init_Is'):
                banlance_reentrancy=v
    func_params_value_reentrancy={}
    if(dict_reentrancy):
        for func in g_abi:
            inputs=func['inputs']
            #print(inputs)
            ...
            for dic in inputs:
                ...
                for k,v in dict_reentrancy.items():
                    ...
                    if(str(dic['name'])==k):
                        d1[k]=v
            func_params_value_reentrancy[str(name)]=d1
    print("1")
    print(func_params_value_reentrancy)
    # for k2,v2 in g_src_map.func_name_to_params.items():
    #     #print type(str(k2)),str(k2),type(v2),v2
    #     #l=[]
    #     d1={}
    #     for dic in v2:
    #
    #         d1[str(dic['name'])]=None
    #         for k1,v1 in dict_reentrancy.items():
    #             #print type(dic),dic
    #             # print(str(dic['name']),k1)
    #             if(str(dic['name'])==k1):
    #                 # print("-------------------")
    #                 d1[k1]=v1
    #                 # l.append(l1)
    #                 # print(k1,v1)
    #             # else:
    #             #     l.append([str(dic['name']),0])
    #             #     print(str(dic['name']))
    #         #l.append(d1)
    #     func_params_value_reentrancy[str(k2)]=d1
    #     # print(func_params_value[k2],k2)
    #print(func_params_value_reentrancy)
    ...
    return func_params_value_reentrancy
...
def to_bytes(n, length, endianess='big'):
    #a function to emulates the to_bytes API
    h = '%x' % n
    s = ('0'*(len(h) % 2) + h).zfill(length*2).decode('hex')
    return s if endianess == 'big' else s[::-1]

def expandall(s,stype,default=32):
    if(s!=None):
        if(stype=='string'):
            m=struct.pack('32s',s.encode('utf-8'))
        elif(stype=='address'):
            m=to_bytes(long(s),32,'little')
        elif(stype.startswith('uint') or stype.startswith('int')):
            m=to_bytes(long(s),32,'big')
        elif(stype=='bool'):
            if(long(s)>0):
                m=to_bytes(1,32,'big')
            else:
             ...
             m=to_bytes(long(s),default,'big')
    else:
        m=to_bytes(0,default,'big')
    return m

def adddynamic(s,stype):
    if(stype.endswith('[]')):
        return 1
    elif(stype=='string'):
        return len(s)
...
def writearg(dictin,abi,name):
    global g_contract_name
    funclist=dictin[g_contract_name]
    tmppath=os.getcwd()
    tmppath=tmppath+'/'+g_contract_name
    k1=open(tmppath+'/'+"dynamicIdx.dat","wb")
    k2=open(tmppath+'/'+name,"wb")
    dynamicnum=0
    dynamiclens=[]
    argbytes=[]


    argbytes.append(expandall(dictin["banlance"],'uint256'))
    argbytes.append(expandall(dictin["timestamp"],'',16))
    argbytes.append(expandall(dictin["blocknumber"],'',16))

    for func in abi:
        if func['type']=='function' and func['name'] in funclist:
            name=func['name']
            inputarg=func['inputs']
            for arg in inputarg:
                if(arg['name'] in funclist[name] and funclist[name][arg['name']]!=None):
                    argbyte=expandall(funclist[name][arg['name']],arg['type'])
                    argbytes.append(argbyte)
                    dy=adddynamic(funclist[name][arg['name']],arg['type'])
                    if(dy>0):
                        dynamiclens.append(struct.pack('B',int(dy)))
                        dynamicnum+=1

    for argbyte in argbytes:
        k2.write(argbyte)
    k1.write(struct.pack('B',dynamicnum))
    for dylen in dynamiclens:
        k1.write(dylen)
    k1.close()
    k2.close()
###

def seek_generate(func_params_value_timestamp,func_params_value_reentrancy):
    global g_contract_name
    global banlance
    global timestamp
    global blocknumber
    ...
    banlance=None
    flag=0
    if(banlance_timestamp):
        if(banlance_reentrancy==None):
            banlance=banlance_timestamp
        else:
            flag=1
    else:
        if(banlance_reentrancy):
            banlance=banlance_reentrancy

    list=[]
    dic={}
    if(func_params_value_timestamp=={}):
        if(func_params_value_reentrancy):
            list.append(func_params_value_reentrancy)
    else:
        if(func_params_value_reentrancy=={}):
            list.append(func_params_value_timestamp)
        else:
            for k in func_params_value_timestamp.keys():
                dic[k]={}
            list.append(dic)
            for k1,v1 in func_params_value_timestamp.items():
                for k2,v2  in v1.items():
                    if(v2==None):
                        if(func_params_value_reentrancy[k1][k2]==None):
                            for d in list:
                                d[k1][k2]=None
                        else:
                            for d in list:
                                d[k1][k2]=func_params_value_reentrancy[k1][k2]
                    else:
                        if(func_params_value_reentrancy[k1][k2]==None):
                            for d in list:
                                d[k1][k2]=func_params_value_timestamp[k1][k2]
                        else:
                            d={}
                            for i in range(len(list)):
                                d=copy.deepcopy(list[i])
                                list[i][k1][k2]=v2
                                d[k1][k2]=func_params_value_reentrancy[k1][k2]
                                list.append(d)
    func_argcs_list=[]
    ...
    if(flag):
        ...
        for i in range(len(list)):
            d[g_contract_name]=list[i]
            d['timestamp']=timestamp
            d['blocknumber']=blocknumber
            d['banlance']=banlance_timestamp
            dic=copy.deepcopy(d)

            dic['banlance']=banlance_reentrancy
            func_argcs_list.append(d)
            func_argcs_list.append(dic)
            d={}
    else:
        for i in range(len(list)):
            d[g_contract_name]=list[i]
            ...
            d['banlance']=banlance
            func_argcs_list.append(d)
    for i in range(len(func_argcs_list)):
        print(func_argcs_list[i])
    json_str = json.dumps(func_argcs_list)
    print(func_argcs_list)
    if((os.path.exists(g_contract_name))==False):
        tmppath=os.getcwd()
        tmppath=tmppath+'/'+g_contract_name
        os.mkdir(tmppath)
        for i in range(len(func_argcs_list)):
            name="testcase"+str(i+1)+".dat"
            writearg(func_argcs_list[i],g_abi,name)
        k3=open(tmppath+'/'+"testcaseCount.dat","wb")
        m=to_bytes(long(len(func_argcs_list)),1,'big')
        k3.write(m)
        k3.close()
def detect_vulnerabilities():
    ...
    if instructions:
        ...
        if g_src_map:
            detect_integer_underflow()######1
            detect_integer_overflow()#####2
            detect_parity_multisig_bug_2()

        log.debug("Checking for Callstack attack...")
        detect_callstack_attack()#####ME2

        if global_params.REPORT_MODE:
            rfile.write(str(total_no_of_paths) + "\n")

        detect_money_concurrency()#####TOD1
        func_params_value_timestamp=detect_time_dependency()#####TD3
        ...
        func_params_value_reentrancy=detect_reentrancy()#####R3

        if global_params.CHECK_ASSERTIONS:
            if g_src_map:
                detect_assertion_failure()###3
            else:
        ...
        seek_generate(func_params_value_timestamp,func_params_value_reentrancy)
...
def run(disasm_file=None, source_file=None, source_map=None,ABI=None,contract_name=None):
    global g_disasm_file
    global g_source_file
    global g_src_map
    global results

...
def check_reentrancy_bug(path_conditions_and_vars, stack, global_state,g_src_map):
    path_condition = path_conditions_and_vars["path_condition"]
    new_path_condition = []
    for expr in path_condition:
        if not is_expr(expr):
            continue
        list_vars = get_vars(expr)##Iv
    ...
    if(ret_val):
        ...
        # print(new_path_condition)
        # print(stack[0]>2300)
        # print(stack[2]>BitVec('Iv',256))
        a=solver.model()
        s=str(a)
        s=s[1:]
        list=s.split( )
...
def update_analysis(analysis, opcode, stack, mem, global_state, path_conditions_and_vars, solver,g_src_map):
    dict={}
    ...
    if opcode == "CALL":
        ...
        reentrancy_result,dict = check_reentrancy_bug(path_conditions_and_vars, stack, global_state,g_src_map)

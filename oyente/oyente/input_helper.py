...
class InputHelper:
    ...
    def get_inputs(self):
        ...
        if self.input_type == InputHelper.BYTECODE:
            ...
            abi_strs = self._get_abi_strs() ##
            abi=[]
            Name=None
            for contract,_ in abi_strs:
                c_source, cname = contract.split(':')
                name=c_source.split('/')[-1]
                name_sol=name.split('.')[0]
                if(cname==name_sol):
                    abi=_
                    Name=name_sol
    ...
    ####
    def _get_abi_strs(self):
        cmd = "solc --abi %s %s" % (self.remap, self.source)## -o ../test/abi
        if self.compilation_err:
            out, err = run_command_with_err(cmd)
            err = re.sub(self.root_path, "", err)
        else:
        ...
        return self._extract_abi_str(out, err)

    def _extract_abi_str(self, s, err=''):
        abi_regex = r"\n======= (.*?) =======\nContract JSON ABI \n(.*?)\n"
        abistrs = re.findall(abi_regex, s)
        abistrs = [abistr for abistr in abistrs if abistr[1]]
        if not abistrs:
            if not self.compilation_err:
                logging.critical("Solidity compilation failed. Please use -ce flag to see the detail.")
                if global_params.WEB:
